...
export const tasksListReducer = (state = initialState, action: UserAction): TaskState => {
  switch (action.type) {
    ...
    case taskTypes.EDIT_TASK:
      ...
      const { todos } = state;
      const itemIndex = todos.findIndex(item => item.id === payload.id);

      const slicedTodos = [
        ...todos.slice(0, itemIndex),
        { ...todos[itemIndex], ...payload },
    ...
    case taskTypes.COMPLETED_STATUS:

      const itemId = state.todos.findIndex(item => item.id === action.payload.id);

      const slicedTodos1 = [
        { ...state.todos[itemId], ...action.payload }
      ];
